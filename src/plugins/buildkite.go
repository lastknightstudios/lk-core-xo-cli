...
func _CreatePipeline(name string) {
	...
	var defaultBranch = "master" // what shows in buildkites pipelines dashboards
	var branchConfig = "master"  // buildkite limiting branch builds
	...
	type Pipeline struct {
		...
		BranchConfig  string `json:"branch_configuration"`
	}

	step := Step{"script", "pipeline:", "buildkite-agent pipeline upload"}
	data := Pipeline{Name: name, Repository: repoName, DefaultBranch: defaultBranch, BranchConfig: branchConfig}
...
//curl -H "Authorization: Bearer $TOKEN" "https://api.buildkite.com/v2/organizations/lastknight/pipelines/lk-test-1/builds" \
//  -X "POST" \
//  -F "commit=HEAD" \
//  -F "branch=master" \
